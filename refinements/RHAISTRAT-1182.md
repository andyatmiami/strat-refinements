...
title: Introduce a "Customize" Entry Point in the Model Catalog for Example-Driven
  Model Adaptation & Discoverability of Model Customization
source_rfe: RHAIRFE-1115
jira_key: RHAISTRAT-1182
priority: Normal
status: Refined
refinement_pr_url: null
---
> **Review Summary — REVISE (Score: 5/8)**
>
...
> | Architecture | 2/2 | ✓ |
>
> **Feasibility:** Effort estimate internally inconsistent (sub-items sum to 10-15 sprints vs. stated 6-9). Model weights provisioning mechanism is unresolved — each option (pre-download, lazy-fetch, symlink) has fundamentally different performance and security implications. Disconnected environment support is absent despite being a core platform requirement. Jupyter extension estimate (3-4 sprints) is unrealistic — 5-7 sprints for experienced team.
>
> **Testability:** All 7 acceptance criteria test only the happy path. No failure-mode criteria for: Git unreachable, PVC failure, invalid OCI URI, RBAC denial, disconnected environment. Annotation key names and PVC mount path are undefined — test engineers cannot write assertions. Performance NFR references unmeasured baseline.
>
> **Scope:** Coherent single capability but RHAIRFE-749 overlap is unresolved (could reduce extension scope by 3-5 sprints). Notebook bundle content scope is open-ended (model × workflow matrix undefined). P2 items not explicitly deferred. Architecture review is a scope gate, not just a risk.
>
> **Edit the `## Staff Engineer Input` section below** with corrections, architectural direction, and scope adjustments. Then mark this PR as Ready for Review.

## Business Need (from RFE)
## Summary

Introduce **Customize** as a first-class action in the Model Catalog that routes users directly into curated, runnable customization examples via a preconfigured Workbench and Jupyter extension, **replacing today's user-managed notebook discovery, cloning, and adaptation process with an intentional, example-driven experience.**

Customize improves discoverability and usability of model adaptation workflows—including fine-tuning, embedding tuning for RAG, prompt tuning, evaluation, and synthetic data generation—by guiding users into working examples at the moment of model selection, without introducing additional UI or infrastructure complexity.

## Current State (As-Is)

Today, there is no **Customize** action in the Model Catalog.

To customize a model, users must:

- Discover a model in the Model Catalog

- Deploy the model (often for inference, not customization)

- Manually open a Workbench

- Locate and clone relevant notebooks from external repositories

- Adapt those notebooks to the chosen model and task

Key characteristics of the current flow:

- Customization is not discoverable from the Model Catalog

- Notebooks live outside the product UX

- Users must already know:
...
-
  - which ones apply to a given model

- First-time users must stitch together multiple concepts:
**Model → Deployment → Workbench → Repository → Notebook**

This creates unnecessary friction and raises the barrier to entry for customization workflows.

## Problem Statement

Model customization is a core value proposition, but today it is hidden behind prior knowledge and manual setup.

- Users cannot discover customization workflows at model selection time

- There is no clear entry point for adapting a model to:
...
-
  - downstream applications such as RAG

- Customization workflows feel fragmented across fine-tuning, embeddings, prompt experimentation, evaluation, and data preparation

- Users must manually discover, clone, and adapt notebooks without product guidance

As a result, powerful customization capabilities are underutilized and harder to adopt than necessary.

## Goals

- Make model customization discoverable at the moment of model selection

- Provide a clear, example-driven happy path into customization

- Route users directly into runnable, curated examples

- Support a broad range of customization workflows, including:

-
  - Fine-tuning (OSFT / PEFT / CPT)

-
  - Embedding tuning to enable RAG pipelines

-
  - Prompt tuning and prompt experimentation

-
  - Evaluation workflows (including LLM-as-judge)

-
  - Data processing and synthetic data generation (SDG)

- Keep infrastructure, environment setup, and repository concepts out of the primary UX

- Support both exploratory users and experienced practitioners

## Non-Goals

- Introducing new Workbench onboarding flows

- Replacing advanced, fully manual workflows

- Abstracting notebooks away entirely

- Building a step-by-step configuration wizard in the UI

## Proposed Solution

### 1. Introduce "Customize" as a First-Class Action in Model Catalog

Add a **Customize** button alongside existing actions (e.g., Deploy) in the Model Catalog.

**Intent of Customize:**

> "I want to adapt this model for my data, task, or application."

Customize is not a deployment shortcut or configuration page.

It is a direct entry point into a working customization example.

### 2. Customize Routes Directly to a Preconfigured Workbench

When a user clicks **Customize**:

- They are immediately routed to a **preconfigured Workbench**

- No intermediate capability-selection or overview pages are shown

At creation time, the system already knows:

- The selected model

- That the user intends to customize the model

- The default example workflow to start from

The Workbench is provisioned with:

- A training-ready base image or Jupyter Minimal Data Science base image

- GPU access (where available and required) [for Model Evaluation/Prompt Tuning]
...
Model weights are made available at a known path so notebooks can reference them directly.

### 3. Jupyter Extension for Notebook Discovery and Materialization

Once Jupyter is running, a **Jupyter extension becomes active**.

The extension is responsible for:

- Discovering curated notebook bundles relevant to:

-
  - the selected model

-
  - the customization intent

- Pulling the latest, versioned notebook bundles from Git

- Cloning the selected notebook bundle into the user's workspace

Example notebook bundles may include:

- Instruction fine-tuning (OSFT with Kubeflow Trainer)

- PEFT / LoRA fine-tuning

- Embedding fine-tuning for RAG enablement

- Prompt tuning and prompt evaluation

- Evaluation workflows (including LLM-as-judge)

- Data processing and synthetic data generation (SDG)

Each notebook bundle represents a known-good, product-endorsed starting point.

This ensures users:

- Do not need to discover repositories manually

- Always start from up-to-date examples

- Can focus on adapting workflows rather than assembling them

### 4. Notebooks as the Guided Happy Path

The notebook itself serves as the primary guidance surface:

- Structured, runnable workflows

- Clear assumptions and prerequisites

- Integrated with supported training, evaluation, and data components

- Designed to be extended and customized by advanced users

This reframes notebooks from external artifacts into first-class product entry points.

## UX Principles

- Discoverability at the moment of intent

- Example-first guidance over abstract configuration

- Progressive disclosure without forced choices

- Explicit support for exploration and iteration

## Success Metrics

- Increased usage of customization notebooks originating from the Model Catalog

- Reduced reliance on external documentation to start customization workflows

- Faster time from model discovery to first successful customization run

- Improved user feedback on customization discoverability and usability
...
<!-- To provide corrections, edit the Feature Refinement PR (preferred) or the Staff Engineer Input section below. -->
